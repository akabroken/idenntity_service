...
--------------------------------------------------------------------------------
PRINT ''
PRINT 'Creating Table: terminals'
PRINT ''
--------------------------------------------------------------------------------
GO

IF EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = object_id(N'dbo.terminals')
AND OBJECTPROPERTY(id, N'IsUserTable') = 1)
PRINT 'TABLE terminals ALREADY EXISTS... CREATE TABLE ABORTED'
ELSE
CREATE TABLE [dbo].[terminals](
	[serno] [bigint] NOT NULL PRIMARY KEY,
	[suppserno] [bigint] NOT NULL FOREIGN KEY REFERENCES SUPPLIERS(SERNO),
	[tradserno] [bigint] NOT NULL FOREIGN KEY REFERENCES TERMINALS(SERNO),
...
-------------------------------------------------------------------------------------------
IF EXISTS (SELECT 1 FROM  sys.indexes WHERE object_id  = OBJECT_ID('terminals') AND name = 'idx_terminals_01')
PRINT 'CONSTRAINT idx_terminals_01 ALREADY EXISTS...ALTER TABLE ABORTED'
ELSE
CREATE UNIQUE NONCLUSTERED INDEX [idx_terminals_01] ON [dbo].[terminals]
(
	[msn] ASC
...
-------------------------------------------------------------------------------------------
GRANT SELECT, INSERT, DELETE, UPDATE ON terminals TO tims_private
GO
-------------------------------------------------------------------------------------------
IF EXISTS (SELECT 1 FROM  sys.sequences WHERE name = 'terminals_serno_seq')
    PRINT 'SEQUENCE tims.terminals_serno_seq ALREADY EXISTS...'
ELSE
CREATE SEQUENCE tims.terminals_serno_seq  
    START WITH 1  
    INCREMENT BY 1 ;  
GO 
PRINT 'Created: terminals_serno_seq SEQUENCE'
-------------------------------------------------------------------------------------------

PRINT 'Creating: trg_terminals_INSERT trigger'
DROP TRIGGER [dbo].[trg_terminals_INSERT] 
GO
CREATE  TRIGGER [dbo].[trg_terminals_INSERT] 
   ON  [dbo].[terminals]
   AFTER INSERT
AS 
BEGIN
SET NOCOUNT ON 
	  ...
	  declare @tab varchar(30) = 'terminals';
 ...
 END
GO
PRINT 'Created: trg_terminals_INSERT trigger'
-----------------------------------------------------------------------------------------------
PRINT 'Creating: trg_terminals_UPDATE trigger'
DROP TRIGGER [dbo].[trg_terminals_UPDATE] 
GO
CREATE  TRIGGER [dbo].[trg_terminals_UPDATE] 
   ON  [dbo].[terminals]
   AFTER UPDATE
AS 
BEGIN
SET NOCOUNT ON 
 ...
 END
GO
PRINT 'Created: trg_terminals_UPDATE trigger'

---------------------------------------------------------------------------------------------------------------------------------------------
PRINT 'Creating: trg_terminals_DELETE trigger'
DROP TRIGGER [dbo].[trg_terminals_DELETE] 
GO
CREATE  TRIGGER [dbo].[trg_terminals_DELETE] 
   ON  [dbo].[terminals]
   AFTER DELETE
AS 
BEGIN
SET NOCOUNT ON 
 ...
 END
GO
PRINT 'Created: trg_terminals_DELETE trigger'
--------------------------------------------------------------------------------------------------------------------------------------------
